...
class Generate_Meal_Plan_Job
{
    public function run($job)
    {
        ...
        $meal_plan_details = $this->create_meal_plan($user_details);
...
//        $meal_plan_day = 'Monday';
        ...
        if (!empty($meal_plan)) {
            ...
            if (empty($meal_plan)) {
                $meal_plan = self::fix_malformed_json($meal_plan);
            ...
            if (!empty($meal_plan)) {
                ...
                error_log('MEALS IMPORTING SUCCESSFUL!!!!');
            } else {
                error_log('MALFORMED JSON!!!! ' . print_r($meal_plan_details, true));
            }
        } else {
...
//        die();
    ...
    function create_meal_plan($user_details)
    {
        extract($user_details);

        $random_dayNumber = rand(0, 6);

        $daysOfWeek = array ('Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday');
        $random_day = $daysOfWeek[$random_dayNumber];
...
//                ]
        ...
        if (isset($completion['choices'][0]['finish_reason']) && $completion['choices'][0]['finish_reason'] !== 'stop') {
            error_log('Malformed meal plan: ' . $content);
        }
