...
class Growtype_Art_Database_Optimize
{
    public static function clean_duplicate_settings()
    {
        ...
        $records = $wpdb->get_results("SELECT * FROM wp_growtype_art_model_settings", ARRAY_A);
        ...
        foreach ($records as $record) {
            if (!isset($filtered_records[$record['model_id']][$record['meta_key']])) {
                ...
                $wpdb->delete('wp_growtype_art_model_settings', array ('id' => $record['id']));
            }
        }

        $records = $wpdb->get_results("SELECT * FROM wp_growtype_art_image_settings", ARRAY_A);
        ...
        foreach ($records as $record) {
            if (!isset($filtered_records[$record['image_id']][$record['meta_key']])) {
                ...
                $wpdb->delete('wp_growtype_art_image_settings', array ('id' => $record['id']));
            }
        }
...
//        $records = $wpdb->get_results("SELECT * FROM wp_growtype_art_image_settings", ARRAY_A);
        ...
        //            $image = growtype_art_get_image_details($record['image_id']);
//            $image_model = growtype_art_get_image_model_details($record['image_id']);
//
//            if (empty($image) || empty($image_model)) {
//                Growtype_Art_Crud::delete_image($record['image_id']);
//                continue;
//            }
...
//        dd('done');
    ...
    public static function sync_models()
    {
        ...
        foreach ($images as $image) {
            ...
            if (empty($model_image)) {
                ...
                $model = Growtype_Art_Database_Crud::get_single_record(Growtype_Art_Database::MODELS_TABLE, [
                    [
                        'key' => 'image_folder',
                        'values' => [$image['folder']],
                    ]
        ...
        foreach ($images as $image) {
            ...
            if (!isset($unique_images[$uniqeu_key])) {
                ...
                if (empty($image_details['settings'])) {
                    ...
                    Growtype_Art_Database_Crud::delete_records(Growtype_Art_Database::IMAGES_TABLE, [$unique_images[$uniqeu_key]['id']]);
                    continue;
                }
            }

            $image_url = growtype_art_build_public_image_url($image);

            if (!file_exists($image_url)) {
                $alternative_extension = $image['extension'] === 'jpg' ? 'png' : 'jpg';
                $image_url = growtype_art_get_upload_dir() . '/' . $image['folder'] . '/' . $image['name'] . '.' . $alternative_extension;
    ...
    public static function sync_local_images($generator = 'models')
    {
         ...
         */
        $upload_dir = growtype_art_get_upload_dir() . '/' . $generator;
...
//        $folders = array_slice($folders, 100, 150);
        ...
        foreach ($folders as $folder) {
            ...
            foreach ($folder_images as $folder_image) {
                $img_path = str_replace(growtype_art_get_upload_dir() . '/', '', $folder_image);
                ...
                $image = Growtype_Art_Database_Crud::get_records(Growtype_Art_Database::IMAGES_TABLE, [
                    [
                        'key' => 'name',
                        'value' => $img_name,
                    ],
                ...
                if (empty($image)) {
                    ...
                    Growtype_Art_Database_Crud::insert_record(Growtype_Art_Database::IMAGES_TABLE, [
                        'name' => $img_name,
                        'extension' => $img_ext,
                        'width' => $size[0],
                        'height' => $size[1],
...
//        $folders = array_slice($folders, 0, 500);
        ...
        foreach ($folders as $folder) {
            ...
            foreach ($folder_images as $folder_image) {
                ...
                if ($size[0] < 650) {
                    Growtype_Cron_Jobs::create_if_not_exists('upscale-image-local', json_encode([
                        'path' => $folder_image,
                        'max_width' => $max_width,
                    ]), $delay += 1);
                }
    ...
    public static function get_images_colors()
    {
        ...
        $table = Growtype_Art_Database::IMAGE_SETTINGS_TABLE;
...
//            growtype_art_init_job('model-assign-categories', json_encode([
//                'model_id' => $model['id']
//            ]), 1);
        }
    }
